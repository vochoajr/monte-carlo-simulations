...
def makePlots():
	...
	fig_x2.text(0.1, 1, "mean = " +str(mean_e2))
	fig_x2.text(0.1, 0.75, "std = " +str(stddev_e2))
	fig_x2.text(0.1, 0.5, "efficiency = " +str(efficiency_e2))
	
	listx_e2a = np.array(listx_e2)
	points = [listx_e2a.min(), listx_e2a.max()]
	func = [2*listx_e2a.min(), 2*listx_e2a.max()]
	fig_x2.plot(points, func, 'g', linewidth=2)
	
	binHeight, binEdges, patches = fig_x3.hist(listx_e3,nBins,normed=True)
	fig_x3.set_title("N = " + str(N_e3))
	fig_x3.text(0.1, 1, "mean = " +str(mean_e3))
	fig_x3.text(0.1, 0.75, "std = " +str(stddev_e3))
	fig_x3.text(0.1, 0.5, "efficiency = " +str(efficiency_e3))
	
	listx_e3a = np.array(listx_e3)
	points = [listx_e3a.min(), listx_e3a.max()]
	func = [2*listx_e3a.min(), 2*listx_e3a.max()]
	fig_x3.plot(points, func, 'g', linewidth=2)
	
	binHeight, binEdges, patches = fig_x5.hist(listx_e5,nBins,normed=True)
	fig_x5.set_title("N = " + str(N_e5))
	fig_x5.text(0.1, 1, "mean = " +str(mean_e5))
	fig_x5.text(0.1, 0.75, "std = " +str(stddev_e5))
	fig_x5.text(0.1, 0.5, "efficiency = " +str(efficiency_e5))
	
	listx_e5a = np.array(listx_e5)
	points = [listx_e5a.min(), listx_e5a.max()]
	func = [2*listx_e5a.min(), 2*listx_e5a.max()]
	fig_x5.plot(points, func, 'g', linewidth=2)
